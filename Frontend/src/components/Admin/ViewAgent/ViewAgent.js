...
function ViewAgent() {
  ...
  const agentsRows = agents.map((agent, index) => {
    ...
    return (
      <tr style={{ color: 'white' }}>
        ...
        {role == "admin" && (
          <td> <button type="button" className="btn btn-danger" onClick={(e) => { handleupdate(e) }} >Update Content</button></td>)}
  ...
  const handleupdate = async (e) => {
    ...
    //     return;
    //   });
    navigateObject(`/admindashboard/updateAgent/${id}`)
  }
  const handleDelete = async (e) => {
    ...
    if (answer) {
      let response = await axios.delete(`http://localhost:8080/agent/deletebyid/${id}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
    ...
    <>
      <div className='container' style={{ minHeight: "100vh" }} >
        <div style={{ overflowX: 'scroll', }}>
          <p style={{ color: 'white' }}><h1>View Agents</h1></p>
          {role == "admin" && (
